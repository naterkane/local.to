<?php
	...
	$selenium->write('verifyTextPresent', '1 Members');
	...
	//delete one invite
	$selenium->write('clickAndWait', 'xpath=id(\'invites_table\')/tbody/tr[3]/td[4]/a');
	...
	$selenium->write('verifyTextNotPresent', $email3);		
	$selenium->write('verifyTextPresent', $email4);
	$selenium->write('verifyTextPresent', $email5);	
	...
	$selenium->write('verifyValue', 'testing_count', $count + 11);	
	$selenium->write('verifyTextPresent', 'The invitation could not be deleted.');
	//sign up as a non regitered user
	$selenium->signout();
	$selenium->openPage('/groups/accept/${key1}');
	$selenium->write('type', 'username', $name2);
	$selenium->write('type', 'realname', $name2);	
	...
	$selenium->write('verifyValue', 'testing_count', $count + 16);
	$selenium->write('verifyTextPresent', '2 Members');
	$selenium->openPage('/home');
	$selenium->write('verifyTextPresent', $group);
	//sign up as a pre-existing user
	$selenium->signOut();	
	$selenium->signUp($name3, $password3, $email4);
